...
app.post("/login", async (req, res) => {
    try {
        ...
        if (user && user.password === password) {
            return res.status(200).json({ success: true, role: "messowner", redirect: "messupdate.html" });
        }
...
app.post("/register-mess", async (req, res) => {
    try {
        const { mess_name, contact_number, mess_time_menu, mess_address, mess_type, monthly_cost, additional_note } = req.body;

        // ✅ Check if all required fields are provided
        if (!mess_name || !contact_number || !mess_time_menu || !mess_address || !mess_type || !monthly_cost) {
            return res.status(400).json({ error: "All fields are required except additional note" });
        }
        ...
        if (existingMess) {
            return res.status(400).json({ error: "Mess with this name already exists" });
        }

        // ✅ Save new mess profile
        const newMess = new MessProfile({ mess_name, contact_number, mess_time_menu, mess_address, mess_type, monthly_cost, additional_note });
        await newMess.save();

        console.log("✅ New Mess Registered:", mess_name);

        res.json({ message: "Mess Registered Successfully!", redirect: "somepage.html" }); // Optional redirect
...
app.put("/messprofiles/update", async (req, res) => {
    try {
        const { mess_name, mess_address, contact_number, mess_time_menu, mess_type, monthly_cost, additional_note } = req.body;
        ...
        if (result.modifiedCount === 0) {
            return res.status(400).json({ error: "Update failed. No changes detected." });
        }
